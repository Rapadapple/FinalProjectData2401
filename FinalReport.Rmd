...
library(plotly)
library(shiny)
>>>>>>> fd374ce8d87b7809e9f5e3172dc2c8bf31b3ff62
```
...
```{r}
fig3 = energy %>% filter(producer == "Total Electric Power Industry", state != "US-TOTAL", year != 2022, source == c("Solar Thermal and Photovoltaic", "Petroleum", "Wind")) %>% group_by(Date, source) %>% 
>>>>>>> fd374ce8d87b7809e9f5e3172dc2c8bf31b3ff62
  summarize(total_generation = sum(generation)) %>% mutate(total_generation = total_generation / 1000000) %>%
  ggplot() +
  geom_line(aes(x= Date, y= total_generation, color = source)) +
...
ggplotly(fig3, tooltip = c("x", "y", "source"))
```
...
server = function(input, output){
  ...
  output$totalGenPlot = renderPlotly({
    ...
    geom_smooth(aes(x = Date, y= total_generation, color = source)) +
    labs(title = "Total Power Generation Over Time", x = "Date", y = "Power Generation(TWH)")
    ggplotly(fig2, tooltip = c("x", "y", "source", "fit"))
  })
}
