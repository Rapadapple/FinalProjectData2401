...
This data was originally a CSV so we used the read_csv function to turn that CSV into a data set and stored it into an object called "energy". The original column names were a hassle to type so we created new column names. The energy data set also contained an erroneous ID column that was not necessary so we deleted it. Lastly we created a new column called Date that was a combination of the month and year columns so that we could create time series graphs.
...
```
## In Conclusion
- Fossil fuels have been a huge detriment to our environment and we want to educate the audience with insight towards energy's evolution to combat fossil fuels and possibly influence one's decision to enact upon such information (with the exception of traffic blockers and vandals).
- We ultimately see in this analysis the major decline of coal, the budding rise of green energy, and the titanic gain in natural gas. The US is headed towards a better direction and, though not the most ideal outcomes have been seen, we are in much better shape than in times past and making the great steps for a more sustainable future.
- For general purposes we this dataset is fit to serve, but there is much more room for discovery within. For instance there is more room for analysis on such as deeper comparisons between energy sources and US comparisons to states. Another possibility is the ability for one to dive deeper and hyper-focus on a states particular energy needs. Perhaps an analysis of industries and their differentiation would be highly informative as well. The only constraint is time and benefit. 
